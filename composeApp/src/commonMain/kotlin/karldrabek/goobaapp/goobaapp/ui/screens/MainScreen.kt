...
import karldrabek.goobaapp.goobaapp.backend.makeTask
        ...
        if (dataConst != null) {
            EditPopup(
                ...
                onUpdateData = {
                    ...
                    editTask = null
                    updateTask(makeTask(editTaskConst, it))
                },
                onDeleteTask = {
                    when (editTaskConst) {
                        TaskType.MORNING_FOOD -> {
                            deleteTask(makeTask(TaskType.MORNING_FOOD, dataConst))
                            morningFoodData = null
                        }

                        TaskType.EVENING_FOOD -> {
                            deleteTask(makeTask(TaskType.EVENING_FOOD, dataConst))
                            eveningFoodData = null
                        }

                        TaskType.SCOOP_POOP -> {
                            deleteTask(makeTask(TaskType.SCOOP_POOP, dataConst))
                            poopScoopData = null
                        }
                    }
                    editTask = null
...
// TODO generalize this and move it to utils
@Composable
private fun Header(
    userName: String,
    onOpenSettings: () -> Unit,
        ...
        if (data == null) {
            OutlinedButton(
                onClick = {
                    ...
                    registerTask(makeTask(taskType, user, clockSnap))
    ...
    PopUp {
        Box {
            ...
            Row(
                ...
                IconButton(
                    onClick = { onUpdateData(EventCompletedData(user, valuesToTime(timeFormat))) },
                ) {
